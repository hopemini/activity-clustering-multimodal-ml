...
class Trainer(object):
    ...
    def _train_batch(self, input_variable, input_lengths, input_part, target_variable, target_part,
                     model, teacher_forcing_ratio):
        loss = self.loss
        # Forward propagation
        decoder_outputs, decoder_hidden, other = model(input_variable, input_lengths, input_part,
                                                       target_variable, target_part,
    ...
    def _train_epoches(self, data, model, n_epochs, start_epoch, start_step,
        ...
        for epoch in range(start_epoch, n_epochs + 1):
            ...
            for batch in batch_generator:
                ...
                input_partitions, _  = getattr(batch, 'srcp')
                target_variables = getattr(batch, 'tgt')
                target_partitions = getattr(batch, 'tgtp')

                loss = self._train_batch(input_variables, input_lengths.tolist(), input_partitions,
                        target_variables, target_partitions,  model, teacher_forcing_ratio)
