...
for _iter in range(iters):
    ...
    save_path = "../../full_data/seq2seq_" + str(_iter) + "/"
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    save_path = save_path + "seq2seq_" + str(_iter)
    ...
    # Dataset Load
    lines_23 = open('data/all_data.txt').read().strip().split('\n')
    ...
    try:
        ...
        np.save(save_path + "_data.npy", rsult)
        name_dic["name"] = name
        name_json = json.dumps(name_dic)
        name_file = open(save_path + "_names.json","w")
